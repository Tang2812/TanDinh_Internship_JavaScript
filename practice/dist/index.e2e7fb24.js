const FormInputController = {
    ...
    handleLoanValue: function(propertyValue, loanAmount, loanTerm, interestRate, disbursementDate) {
        ...
        if (object.loanTerm < 2) {
            ...
            for(let i = 1; i <= object.loanTerm; i++){
                ...
                if (minMonthlyPayment > resultRecord.toralPrincipalAndInterest) minMonthlyPayment = resultRecord.toralPrincipalAndInterest;
                if (maxMonthlyPayment < resultRecord.toralPrincipalAndInterest) maxMonthlyPayment = resultRecord.toralPrincipalAndInterest;
                result.push(resultRecord);
            }
        }
    ...
    validateValue: function(valueOfPropertyValue, valueOfLoanAmount, valueOfLoanTerm, valueOfInterestRate, valueOfDisbursementDate) {
        ...
        } else inputErrors.errorMessages.push('');
    ...
    reformater: function(value) {
        if (!isNaN(value) && value !== '') return Number(value).toLocaleString('en');
    },
    ...
    calculateRepaymentDate: function(date) {
        ...
        // check Day
        if (newDay > daysOfMonth) newDay = daysOfMonth;
...
//# sourceMappingURL=index.e2e7fb24.js.map
